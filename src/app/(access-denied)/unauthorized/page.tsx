  ...
  return (
    <div className="bg-[#1D1F24]  bg-no-repeat bg-top bg-fixed relative flex flex-col items-center min-h-[100vh] overflow-hidden place-content-center">
      <div className="bg-top bg-no-repeat max-w-[768px] flex flex-col justify-center items-center bg-[#1D1F24]">
        ...
        <h1 className="w-full flex text-center flex-col text-secondary m-0 font-bold -font-grotesk text-2xl sm:text-[40px]/[60px] -tracking-[0.02em]">
          Your session has expired.
          <span className="text-center text-secondary m-0 font-normal -font-grotesk text-[20px]/[30px]">
            Please log in again to continue.
          </span>
        </h1>
        <div className="w-full flex flex-col sm:flex-row gap-4 justify-center items-center mt-8">
          <Button
            onClick={() => {
              dispatch(logout())
              router.push("/")
            }}
            className="w-50 text-[#1d1f24]"
          >
            Logout
          </Button>
        </div>
