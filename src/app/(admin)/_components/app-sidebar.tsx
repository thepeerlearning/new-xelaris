    ...
    <div className="bg-[#1D1F24] flex flex-col min-h-screen">
      <div className="lg-md:hidden">
        <header className="flex h-16 items-center border-b px-4 bg-[#1D1F24]">
          <MobileNav user={user} />
          <h1 className="ml-2 text-xl font-semibold -font-grotesk text-white">
            Admin Portal
          </h1>
        </header>
        ...
        <SidebarProvider>
          ...
          <SidebarInset>
            <header className="flex h-16 items-center border-b border-[#E5E9EB]/20 px-4 bg-[#1D1F24]">
              ...
              <h1 className="text-xl font-semibold -font-grotesk text-white">
                Admin Portal
              </h1>
            </header>
            <main className="p-4 bg-[#1D1F24]">{children}</main>
      ...
      >
        <div key={user?.id} className={`bg-transparent flex items-center`}>
          <Avatar className="w-8 h-8">
            ...
            <AvatarFallback className="bg-[#F2F4F7] text-black font-medium -font-grotesk text-sm">
              {getInitials(user?.name?.trim())}
            </AvatarFallback>
          </Avatar>
          <div className="flex-1 space-y-1">
            <p
              className={
                "text-black font-medium -font-grotesk text-sm capitalize"
              }
            >
              {user?.name}
            </p>
            <p className={"text-secondary font-normal -font-grotesk text-sm"}>
              {user?.email}
            </p>
          </div>
