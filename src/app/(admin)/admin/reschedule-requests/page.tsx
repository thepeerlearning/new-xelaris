      ...
      ? data?.map((row: any, i: number) => ({
          ...
          status:
            row.status.toLowerCase() === "pending" ? (
              <div className="flex h-[30px] w-[80px] items-center justify-center gap-1 rounded-full border border-solid border-primary/10 bg-primary/10 px-[6px] py-[2px] -font-grotesk text-xs/[18px] font-medium capitalize text-primary">
                {row.status}
              </div>
            ) : (
              <div className="flex h-[30px] w-[80px] items-center justify-center gap-1 rounded-full border border-solid border-[#ABEFC6] bg-[#ECFDF3] px-[6px] py-[2px] -font-grotesk text-xs/[18px] font-medium capitalize text-[#067647]">
                {row.status}
              </div>
            ),
